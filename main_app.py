...
if __name__ == '__main__':
    if login():
        ...
        # 時間軸リストの作成
        count_time = datetime.datetime(
            year=1900, month=1, day=1, hour=8, minute=scale[sel_scale][0])
        list_90 = []
        for i in range(scale[sel_scale][1]):
            list_90.append(count_time)
            count_time += datetime.timedelta(minutes=scale[sel_scale][0])
        li = ['開催場', 'グレード', '開催区分']+[str(list_90[i].time().strftime("%H:%M"))
                                      for i in range(scale[sel_scale][1])]
        ...
        uploaded_file = st.sidebar.file_uploader(
            '場外発売管理システムの締切時刻一覧表をアップロードしてください。ファイル形式はxlsxに変更してください', type='xlsx')
        ...
        if os.path.isfile('./data/' + file_name):
            ...
            kaisai_date = str(df[option].iloc[1, 1])[2:] + \
                '('+str(df[option].iloc[0, 15])+')'
            ...
            df[option].columns = ['開催場', 'グレード', '開催区分', '1R', '2R', '3R',
                                  '4R', '5R', '6R', '7R', '8R', '9R', '10R', '11R', '12R']
            ...
            duplicates = list(
                set([x for x in list_ALL if list_ALL.count(x) > 1]))
            duplicates.sort()
            ...
            st.dataframe(df2.style.applymap(
                lambda x: 'background-color: yellow' if x in duplicates else 'background-color: white'),height=(int(kaisai_count.replace('場',''))+1)*35)
            ...
            # Excel ファイルに書き込む
            df2.style.applymap(lambda x: 'background-color: yellow' if x in duplicates else 'background-color: white').to_excel(
                'output.xlsx', index=False, sheet_name=option)

            # ファイルをダウンロードする
            with open('output.xlsx', 'rb') as f:
                bytes = f.read()
                st.download_button('Download Excel', data=bytes, file_name='output.xlsx',
            ...
            with tab1:
                ...
                st.write(
                    f'<style>table {{border-collapse: collapse;}} table, th, td {{border: 1px solid black; padding: 5px;}}</style>', unsafe_allow_html=True)
                st.dataframe(dfM.style.applymap(
                    lambda x: 'background-color: yellow' if x in duplicates else 'background-color: white'))
                # Excel ファイルに書き込む
                dfM.style.applymap(lambda x: 'background-color: yellow' if x in duplicates else 'background-color: white').to_excel(
                    'output.xlsx', index=False, sheet_name=option)
                # ファイルをダウンロードする
                with open('output.xlsx', 'rb') as f:
                    bytes = f.read()
                    st.download_button('Download Excel', data=bytes, file_name='output_morning.xlsx',
                                       mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
            ...
            with tab2:
                ...
                st.write(
                    f'<style>table {{border-collapse: collapse;}} table, th, td {{border: 1px solid black; padding: 5px;}}</style>', unsafe_allow_html=True)
                st.dataframe(dfD.style.applymap(
                    lambda x: 'background-color: yellow' if x in duplicates else 'background-color: white'))
                # Excel ファイルに書き込む
                dfD.style.applymap(lambda x: 'background-color: yellow' if x in duplicates else 'background-color: white').to_excel(
                    'output.xlsx', index=False, sheet_name=option)
                # ファイルをダウンロードする
                with open('output.xlsx', 'rb') as f:
                    bytes = f.read()
                    st.download_button('Download Excel', data=bytes, file_name='output_day.xlsx',
                                       mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
            ...
            with tab3:
                ...
                st.write(
                    f'<style>table {{border-collapse: collapse;}} table, th, td {{border: 1px solid black; padding: 5px;}}</style>', unsafe_allow_html=True)
                st.dataframe(dfN.style.applymap(
                    lambda x: 'background-color: yellow' if x in duplicates else 'background-color: white'))
                # Excel ファイルに書き込む
                dfN.style.applymap(lambda x: 'background-color: yellow' if x in duplicates else 'background-color: white').to_excel(
                    'output.xlsx', index=False, sheet_name=option)
                # ファイルをダウンロードする
                with open('output.xlsx', 'rb') as f:
                    bytes = f.read()
                    st.download_button('Download Excel', data=bytes, file_name='output_nighter.xlsx',
                                       mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
